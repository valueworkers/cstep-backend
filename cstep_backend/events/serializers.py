...
class StreamRecordingSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        fields = ["id", "started_by", "started_at", "ended_at", "file_url", "status"]
...
class BroadcastSessionSerializer(serializers.ModelSerializer):
    ...
    def get_recordings(self, obj):
        request = self.context.get("request")
        qs = obj.recordings.all()
        is_staff_role = (
            request and request.user.is_authenticated
            and request.user.role in {UserRole.MODERATOR, UserRole.EVENT_ADMIN, UserRole.SUPER_ADMIN}
        )
        if not is_staff_role:
            qs = qs.filter(status=RecordingStatus.READY)
        return StreamRecordingSerializer(qs, many=True).data
